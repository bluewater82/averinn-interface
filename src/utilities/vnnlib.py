...
class VNNLib:
    ...
    def get_num_inputs_outputs(onnx_filename):
        ...
        inp, out, inp_dtype = VNNLib.get_io_nodes(onnx_model)
    ...
    def read_vnnlib_simple(vnnlib_filename, num_inputs, num_outputs):
        ...
        rv.append((VNNLib.make_input_box_dict(num_inputs), [], []))

        lines = VNNLib.read_statements(vnnlib_filename)
        ...
        for line in lines:
            ...
            if groups:
                ...
                for rv_tuple in rv:
                    VNNLib.update_rv_tuple(rv_tuple, op, first, second, num_inputs, num_outputs)
            ...
            for rv_tuple in old_rv:
                for c in conjuncts:
                    ...
                    for i in range(count):
                        ...
                        VNNLib.update_rv_tuple(rv_tuple_copy, op, first, second, num_inputs, num_outputs)
